...
class MSDFGeometry extends THREE__namespace.BufferGeometry {
    font;
    text;
    width;
    alignX;
    ...
    generateGeometry({ uv = true, position = true, charUv = true, index = true, charPosition = true, } = {}) {
        ...
        this.setAttribute('position', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4 * 3), 3));
        this.setAttribute('charUv', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4 * 2), 2));
        if (charPosition)
            this.setAttribute('charPosition', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4 * 3), 3));
        if (uv)
            this.setAttribute('uv', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4 * 2), 2));
        if (index) {
            this.setAttribute('charIndex', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4), 1));
            this.setAttribute('charIndex', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4), 1));
            this.setAttribute('lineCharIndex', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4), 1));
        }
        this.setAttribute('id', new THREE__namespace.BufferAttribute(new Float32Array(numChars * 4), 1));
...
THREE.ShaderChunk.msdftest_fragment = msdftestFragment;
THREE.ShaderChunk.msdftest_pars_fragment = msdftestParsFragment;
THREE.ShaderChunk.msdf_glyph_uv_vertex = msdfcharUvVertex;
THREE.ShaderChunk.msdf_glyph_uv_pars_vertex = msdfcharUvParsVertex;

Object.keys(THREE.ShaderLib).forEach(shaderName => {
    const shaderDef = THREE.ShaderLib[shaderName];
...
exports.Font = Font;
exports.MSDFGeometry = MSDFGeometry;
